...
// ✅ Update Reservation (New Feature)
exports.updateReservation = async (req, res, next) => {
    try {
        const { id } = req.params; // Get reservation ID from URL parameters
        const updates = req.body; // Get updates from request body
        const userId = req.user._id || req.user.id; // User ID from authenticated middleware
        ...
        const reservation = await Reservation.findOneAndUpdate(
            { _id: id, user: userId }, // Find by ID and user ID
            updates,
            { new: true, runValidators: true } // Return updated document and run schema validators
        );

        if (!reservation) {
            // If reservation not found or doesn't belong to the user
            return next(new ErrorHandler("Reservation not found or you don't have permission to update it", 404));
        }
        ...
        console.error("Backend: Error updating reservation:", error);
        // Handle Mongoose validation errors specifically
        if (error.name === 'ValidationError') {
            const messages = Object.values(error.errors).map(val => val.message);
